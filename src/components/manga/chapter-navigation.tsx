...
const ChapterNavButton = ({
  ...
  const handleClick = async () => {
      ...
      if (direction === 'next' && (isAsc ? Number(offset) < total : Number(offset) > 0) && hasMoreChapters) {
        ...
        try {
          ...
          if (chaptersData?.data?.length) {
            ...
            saveReaderContext({ mangaId, offset: newOffset, langFilterValue, langValue, order })
            router.push(`/reader/${selectedChapter.id}`, { scroll: false })
          }
        } catch (error) {
    ...
    saveReaderContext({ mangaId, offset, langFilterValue, langValue, order })
    router.push(`/reader/${chapter.id}`, { scroll: false })
  }
  ...
  if (variant === 'minimal') {
    return (
      <button
        onClick={handleClick}
        disabled={isDisabled}
        className={`p-2 rounded-lg transition-all duration-300 border cursor-pointer ${isDisabled
  ...
  return (
    <button
      onClick={handleClick}
      disabled={isDisabled}
      className={`flex items-center gap-3 px-8 py-4 text-xs font-black uppercase tracking-[0.2em] rounded-2xl transition-all duration-500 shadow-2xl border cursor-pointer group ${isDisabled
